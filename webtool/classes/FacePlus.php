...
class FacePlus{
    ...
    */
    protected function get_outer_id(){
        $limit = 1;
        $find = db_get("webtool_faceplus_outer_id",[],1);
        if(!$find || $find['count'] > $limit){
            $outer_id = time();
            db_insert("webtool_faceplus_outer_id",[
                'outer_id'=>$outer_id,
                'num'=>0
            ]);
            return $outer_id;
    ...
    protected function update_outer_id_num($outer_id,$num = 1){ 
        $find = db_get("webtool_faceplus_outer_id",['outer_id'=>$outer_id],1);
        $num = $find['num']+$num;
        db_update("webtool_faceplus_outer_id",['num'=>$num],['outer_id'=>$outer_id]);
    }
    /**
    * 设置人脸库
    ...
    */
    public function get_detect($file){
        ...
        if(!file_exists($file)){  
            trace("faceplus get_detect:".$file." not exist",'error'); 
    ...
    */
    protected function get_all_outer_id(){
        $all = db_get("webtool_faceplus_outer_id",[]);
        $in = [];
        foreach($all as $v){
            $in[] = $v['outer_id'];
        }
